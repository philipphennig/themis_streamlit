...
with explanations:
    st.markdown(
        """
        The [Themis Mechanism](https://mlg.eng.cam.ac.uk/carl/climate) by [Carl Edward Rasmussen](https://mlg.eng.cam.ac.uk/carl/) is a proposal for a global cooperative framework for addressing climate change. It relies on reciprocity and common commitments. Part of the proposal is a price elicitation process, which   determines a global CO₂e price from a set of heterogeneous national preferences. It selects the global price which maximizes the global CO₂e revenue (a proxy for the climate impact of the coalition) when every country only gets to choose whether to participate (i.e., pay the set price on their emissions) or not. No nation is ever asked to do anything inconsistent with their expressed preferences.

        This app simulates the price elicitation process of the Themis Mechanism. There are two main plots: the Elicitation plot (pair), which produces the Themis price and the Consequences plot, showing the national payments and international transfers for each nation. 
        
        Use the sidebar controls to explore how the mechanism works under different assumptions about the distribution of countries' price preferences and the resulting price and coverage.

        - **Pick your country**: Select the country you want to represent in the mechanism.
        - **Your preferred price**: Set the preferred CO₂e price using the slider. This represents your country's stated preference in the mechanism; once chosen the value is locked in. Sequentially choose prices for other nations to build a global picture.
        - **Sampled preferences**: Prices which are unspecified (not locked in) are drawn randomly, in one of multiple ways. You can choose the between 
        ...
        - **Emissions per capita**: The small plot shows the countries' emissions per capita for reference. These numbers are used to determine the order of countries' preferences in the correlated sampling models (because they correlate with development status and thus likely with willingness to pay), but they do not directly affect the Themis price or coverage.

        The bottom plot shows the consequences in terms of payments per capita for different nations, as a function of average per capita emissions. On the left axis the total amount collected nationally. On the right axis is the net transfer out of each country. The international net transfer is the difference between the national payment times the global faction g, and the Themis payout. For small emitters, this difference is negative, the nations are net beneficiaries. Nation names are indicated at the top, in blue for member nations, and red for non-members.   
