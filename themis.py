...
with explanations:
    st.markdown(
        ...
        - **Overlays**: Use the checkboxes below to show or hide additional information on the plot:
            + *lost coverage* is the fraction of global emissions not covered by the set price. It is equal to 1 minus the share , the global set price itself, and the revenue generated by Themis.
        ...
        - **Emissions pp**: The bottom plot shows the countries' emissions per capita for reference. These numbers are used to determine the order of countries' preferences in the correlated sampling models (because they correlate with development status and thus likely with willingness to pay), but they do not directly affect the Themis price or coverage.
