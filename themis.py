...
fig, ax = plt.subplots()
bars = ax.bar(
    price_preferences, shares, width=1.0, color=rgb.tue_blue, label="countries"
)
ax.axvline(price_preference, color=rgb.tue_blue, lw=0.75, label=f"your price: {price_preference} EUR/tCO2e")

ax2 = ax.twinx()
cov = ax2.plot(xplot, SF, color=rgb.tue_blue, label="achieved coverage")
ax3 = ax.twinx()
# make extra third yaxis:
ax3.spines["right"].set_position(("outward", 30))
gsp = ax3.plot(xplot, xplot, color=rgb.tue_green, label="global set price")

ax4 = ax.twinx()
ax4.spines["right"].set_position(("outward", 70))
tr = ax4.plot(xplot, xplot * SF, color=rgb.tue_red, label="Themis revenue")
...
ax2.yaxis.label.set_color(rgb.tue_blue)
ax3.set_ylabel("global set price $p$ [EUR/tCO2e]")
ax3.yaxis.label.set_color(rgb.tue_green)

ax4.set_ylabel("Themis revenue [EUR/tCO2e]")
ax4.yaxis.label.set_color(rgb.tue_red)
...
ax.axvline(
    x=xplot[max_location], color=rgb.tue_red, linestyle="-", label="Themis price"
)
